...
## 结课报告
在本次加密货币技术的实验课中，我们最初先定义了一个名为 Blockchain 的类，它是一个区块链数据结构的实现。 其中我们实现了constructor(name)方法、longestChain()方法，以此返回当前最长链的区块信息，同时Block类定义了相关的情况属性，包括所属区块链 父块哈希 高度 哈希，我们在后面的实验中又在block中定义了多个函数，比如用来检查区块是否符合难度要求、计算哈希值、设置随机数等等，在这个类添加了各种属性去完善区块链的各种功能。随着课程的深入，新增了UTXO、UTXOPool、Transaction的类，对它们进行与交易相关的功能的实现，完善上述各种文件。最终我们构造一个具有基础功能的模拟比特币的平台。在这个过程中通过学习和搜集资料理解了51%算力攻击，POW共识机制及特点，UTXO模型的原理及设计必要性，cionbase交易的内容、P2P交易以及如何能够更加高效地实现挖矿，同时了解到了比特币网络的新技术，如闪电网络。
总之通过这个实验，我们基本掌握了用JavaScript实现区块链的方法，更加深刻地理解了区块链原理及其比特币交易中的各个流程以及相关知识。
